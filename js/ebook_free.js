...
async function loadFreeEbooks() {    
    const container = document.getElementById("ebook_free");
    try {
...
window.addEventListener('DOMContentLoaded', loadFreeEbooks);
...
async function handleApiSubmit() {
  ...
  const submitBtn = document.querySelector("button[onclick='handleApiSubmit()']");
  ...
  try {
    const res = await fetch(SEND_EMAIL_FOR_SEND_OTP, {
      ...
      body: JSON.stringify({ email, type: "Free-EBooks" })
    });
    ...
    if (res.ok) {
      ...
      document.getElementById("api_status").insertAdjacentHTML("afterend", `
        ...
        <div class="mt-3 d-flex justify-content-between">
          <button type="button" onclick="verifyOtp('${name}', '${email}', '${resource}', '${phone_no}')" 
                  class="btn btn-success w-50 me-2">Verify OTP & Get EBook</button>
          <button type="button" onclick="resendOtp('${email}', '${resource}')" 
...
async function verifyOtp(name, email, resource, mobile_no) {
  ...
  try {
    const res = await fetch(VERIFY_EMAIL_FOR_SEND_OTP, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, email, type: "Free-EBooks", otp , resource, mobile_no})
    });
    const data = await res.json();
    ...
    if (res.ok) {
      ...
      // Reset everything back to original state after a short delay
      setTimeout(() => {
        // Clear form fields
...
async function resendOtp(email, resource) {
  try {
    const res = await fetch(SEND_EMAIL_FOR_SEND_OTP, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
